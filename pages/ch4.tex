...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{CCCC}
      ...
      Models & ACC(\%) & Kappa & SD \\
      \midrule
      LS-Before & \textbf{76.58} & \textbf{0.69} & \textbf{11.45} \\
...
随着模型的逐渐复杂，准确率、Kappa一致性呈现上升趋势，标准差呈现下降趋势，证明了每个模块的有效性。此外，对Inception进行MI-EEG分类任务的针对性改进后得到的BaseNet具有最明显的提升，多尺度密集连接模块、svSE模块、SCoT模块也具有明显的提升，而LSTM的提升不明显。这可能说明了相较于循环神经网络，卷积神经网络在MI-EEG分类任务上具有更大的优势。
...
对于四分类的BCI Competition IV Dataset 2A数据集，实验数据显示：

(1) HA-FuseNet在未进行数据增强的被试内实验中，取得了最优的准确率和Kappa一致性，且准确率标准差低于ShallowConvNet、EEGInception和LMDA-Net，证明了HA-FuseNet具有优秀的MI-EEG分类能力，且对不同的被试具有较好的稳定性；
...
表~\ref{tab:2acomparein}~展示了不同模型进行被试内实验的准确率，表~\ref{tab:2acompareinsd}~展示了被试内实验的Kappa一致性系数和标准差，其中，Kappa一致性系数为九名被试的平均值。需要说明的是，表~\ref{tab:2acomparein}~和~\ref{tab:2acompareinsd}~展示的是未进行数据增强的结果，即对于每名被试，使用数据规模为288的训练集。为了更直观地对数据进行展示，在表~\ref{tab:2acomparein}~和表~\ref{tab:2acompareinsd}~中通过加粗的方式对基础模型中的最优值进行了强调。
...
\begin{table}[ht]
    ...
    %   \caption{子表a}
      ...
      \begin{tabularx}{\textwidth}{cCCCCC}
        ...
        \midrule 
        \textbf{HA-FuseNet}  & 91.67 & 62.85 & 92.36 & 63.54 & 75.00\\
        \textbf{HA-FuseNet(SG)} & 88.81 & 63.89 & 92.01 & 63.54 & 71.88\\
        \bottomrule
      \end{tabularx}
    \end{subtable}
    \begin{subtable}[ht]{\textwidth}
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{CCC}
      ...
      \midrule 
      \textbf{HA-FuseNet} &0.70&11.22\\
      \textbf{HA-FuseNet(SG)} &0.69&11.59\\
      \bottomrule
    \end{tabularx}
\end{table}
...
(1) 对于基准模型，ShallowConvNet取得了最高的平均准确率与Kappa值，但准确率标准差达到了12.15，表明不同被试之间的准确率差异较为明显，性能相对不均衡；
...
对比基准模型和论文提出的模型的实验数据，可以发现HA-FuseNet取得了最高的平均准确率和Kappa值，且在1号被试和6号被试上取得了最高的准确率，同时，HA-FuseNet的准确率标准差低于具有次优平均准确率和Kappa值的ShallowConvNet；使用SG轻量化卷积模块的HA-FuseNet(SG)取得了仅次于ShallowConvNet的准确率和Kappa一致性，其准确率的标准差同样低于ShallowConvNet。相较于HA-FuseNet，HA-FuseNet(SG)的平均准确率仅下降了约0.86个百分点。

实验证明了HA-FuseNet和HA-FuseNet(SG)的有效性，能够在使用小规模数据集的情况下取得较好的性能表现。此外，通过对各项模型的实验结果分析，印证了先前研究中，关于浅层网络在小规模数据集的MI-EEG分类任务中性能表现优良的理论。
...
\begin{table}[ht]
    ...
    %   \caption{子表a}
      ...
      \begin{tabularx}{\textwidth}{CCCCCC}
        ...
        \midrule 
        \textbf{HA-FuseNet} & 92.37 & 65.15 & 94.36 & 70.14 & 77.08\\
        \bottomrule
      \end{tabularx}
    \end{subtable}
    \begin{subtable}[ht]{\textwidth}
    ...
    %   \caption{子表b}
      ...
      \begin{tabularx}{\textwidth}{CCCCCCC}
        ...
        \midrule 
        \textbf{HA-FuseNet} & 67.92 & 93.75 & 83.68 & 87.15 & 81.29 \\
        \bottomrule
      \end{tabularx}
    \end{subtable}
...
\begin{table}[ht]
    ...
    \begin{tabularx}{\textwidth}{CCC}
      ...
      \midrule 
      \textbf{HA-FuseNet} & 0.76 & 10.90 \\
      \bottomrule
    \end{tabularx}
\end{table}

数据显示，除EEGConformer之外，其他模型的平均准确率均有所上升，其中，EEGInception的平均准确率升高了10.79个百分点，EEGResNet升高了约9.9个百分点，DeepConvNet和HA-FuseNet的平均准确率上升幅度则相对较小。这证明了数据增强算法的有效性，可以基于此增加模型学习不同频带数据分布的能力，从而提升模型的性能表现。
...
此外，表~\ref{tab:2acompareagsd}~表明进行数据增强之后，除EEGConformer之外，其他模型的Kappa一致性均有所上升。论文所提出的HA-FuseNet的kappa值达到了0.76，准确率的标准差下降为10.90，仅次于DeepConvNet的10.73和LMDA-Net的10.74，表明HA-FuseNet的跨被试泛化能力得到了进一步的加强。
...
表~\ref{tab:2acomparecross}~展示了各个模型进行被试间实验的准确率结果对比，例如，表格中[ShallowConvNet,1]即表示ShallowConvNet使用其他八名被试的训练集集合，在1号被试的测试集上得到的准确率。表~\ref{tab:2acomparecrosssd}~展示了被试间实验的Kappa系数和标准差结果对比，其中，Kappa系数为九名被试的平均值。被试间对比试验未使用数据增强算法。
...
\begin{table}[ht]
    ...
    %   \caption{子表a}
      ...
      \begin{tabularx}{\textwidth}{CCCCCC}
        ...
        \midrule 
        \textbf{HA-FuseNet}  & 76.13 & 52.79 & 88.89 & 55.13 & 60.42\\
        \bottomrule
      \end{tabularx}
    \end{subtable}
    \begin{subtable}[ht]{\textwidth}
    ...
    %   \caption{子表b}
      ...
      \begin{tabularx}{\textwidth}{CCCCCCC}
        ...
        \midrule 
        \textbf{HA-FuseNet}  & 53.99 & 76.74 & 78.13 & 70.06 & 68.25 \\
        \bottomrule
      \end{tabularx}
    \end{subtable}
...
\begin{table}[ht]
    ...
    %   \caption{子表a}
      ...
      \begin{tabularx}{\textwidth}{CCCCCC}
        ...
        \midrule 
        HA-FuseNet   & 79.17 & 80.00 & 70.83 & 95.39 & 76.92 \\
        \bottomrule
      \end{tabularx}
    \end{subtable}
    \begin{subtable}[ht]{\textwidth}
    ...
    %   \caption{子表b}
      ...
      \begin{tabularx}{\textwidth}{CCCCCCC}
        ...
        \midrule 
        HA-FuseNet   & 75.00 & 69.17 & 66.43 & 64.17 & 75.23 \\
        \bottomrule
      \end{tabularx}
    \end{subtable}
...
\begin{table}[ht]
    ...
    %   \caption{子表b}
      ...
      \begin{tabularx}{\textwidth}{CCCCCCC}
        ...
        \midrule 
        HA-FuseNet  & 82.50 & 80.00 & 72.14 & 71.67 & 76.86\\
        \bottomrule
      \end{tabularx}
    \end{subtable}
