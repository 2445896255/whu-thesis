...
EEG信号具有低信噪比的特性，混杂了大量的噪声和干扰成分，通过结合最大池化操作和平均池化这两种操作，能够能够在最大程度保留EEG信号中关键特征的同时，对噪声起到一定的抑制作用，从而在不同的特征信息之间取得平衡。在取得池化表示\(X'\)后，通过公式~\ref{eq:scots2}~获得键（Key）、查询（Query）、值（Value）矩阵\(K\)、\(Q\)和\(V\)。
